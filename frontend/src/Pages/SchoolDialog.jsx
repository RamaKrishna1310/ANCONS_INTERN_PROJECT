...
export default function SchoolDialog({ open, onClose, schoolData, update }) {
    ...
    useEffect(() => {
        const getCountries = async () => {
            try {
                ...
                setSelectedCountry(schoolData?.country || firstCountry);
            } catch (error) {
                setCountries([]);
            }
        };
    ...
    useEffect(() => {
        const getStates = async () => {
            try {
                ...
                setSelectedCity('');
                setStates(allStates);
                setSelectedState(schoolData?.state || firstState);
            } catch (error) {
                setStates([]);
                setCities([]);
                setSelectedCity('');
    ...
    useEffect(() => {
        const getCities = async () => {
            try {
                ...
                setCities(allCities);
                setSelectedCity(schoolData?.city || firstCity);
            } catch (error) {
                setCities([]);
            }
        };
    ...
    const handleSubmitSchool = (data) => {
        const historyId = schoolData?.id || 0;
        const historyData = {
            type: "school",
    ...
    useEffect(() => {
        reset({
            institutionName: schoolData?.institutionName || "",
            country: schoolData?.country || "",
            city: schoolData?.city || "",
            state: schoolData?.state || "",
            startMonthYear: schoolData?.startMonthYear || "",
            endMonthYear: schoolData?.endMonthYear || "",
            levelOfStudy: schoolData?.levelOfStudy || "",
        });
    }, [schoolData, reset]);

    return (
        <Dialog onClose={onClose} open={open}>
            <form className="flex flex-col pb-4 px-8 gap-4" onSubmit={handleSubmit(handleSubmitSchool)}>
                <DialogTitle style={{ fontWeight: 600, fontSize: 28, color: "#5D4DC9", paddingLeft: "0", paddingBottom: "0" }}>Add School</DialogTitle>
                <table cellPadding={4}>
                    <tbody>
                        <tr>
                            <td>School</td>
